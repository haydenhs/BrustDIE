...
class SonyDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        if self.input_images[str(ratio)[0:3]][ind] is None:
            ...
            bayer = process_bayer(raw) * ratio
            bayer = np.minimum(bayer, 1.0)
            self.input_images[str(ratio)[0:3]][ind] = bayer
            gt_raw = rawpy.imread(gt_path)
            self.gt_images[ind] = process_bayer(gt_raw)
        ...
        if np.random.randint(2, size=1)[0] == 1:
            ...
            gt_patch = np.transpose(gt_patch, (1, 0)).copy()

        input_patch = torch.from_numpy(input_patch).unsqueeze(0)
        gt_patch = torch.from_numpy(gt_patch).unsqueeze(0)
